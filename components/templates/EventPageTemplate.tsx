...
export default function EventPageTemplate({ page, labels, fallbackSponsors }: EventPageTemplateProps) {
  ...
  return (
    <SubpageLayout title={page.title} intro={page.intro}>
      ...
      <h2 className="mb-4 text-2xl font-semibold text-mist">{labels.infoCards}</h2>
      <div className="grid gap-5 md:grid-cols-3">
        {page.infoCards.map((card) => (
          <GlassCard key={`${page.slug}-${card.title}`} className="p-6">
            <p className="text-lg font-semibold text-mist">{card.title}</p>
            <p className="mt-3 text-sm text-slate-200/85">{card.description}</p>
          </GlassCard>
        ))}
      </div>

      <h2 className="mb-4 mt-8 text-2xl font-semibold text-mist">{labels.program}</h2>
      <Timeline items={page.program} />

      <h2 className="mb-4 mt-8 text-2xl font-semibold text-mist">{labels.sponsors}</h2>
